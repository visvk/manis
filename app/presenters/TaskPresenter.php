...
class TaskPresenter extends BasePresenter {
	...
	{if (!$this->user->isAllowed('Default')){
        ...
        }else{$this->flashMessage('Problém pri zmazaní súboru (URL = NULL) alebo súbot nenájedný.', 'error');}
        if (!$this->presenter->isAjax()) {
    ...
    protected function createComponentTaskRegForm() {
        $users = $this->context->database->table('proj_us')->select('user_subj_id,user_subj.user.login')->where('project_id', $this->idproj)->fetchPairs('user_subj_id', 'login');
    ...
    protected function createComponentProj_usRegForm() {
        $users = $this->context->database->table('user_subj')->select('user_subj.id,user.login')
                        ->where('subject_id', $this->project->subject_id)->fetchPairs('id', 'login');
    ...
    protected function createComponentProj_usGrid($name) {
        ...
        $grid->setModel($this->context->database->table('proj_us')
                        ->select('user_subj.user.login, manager')->where('project_id', $this->idproj))
                ->setDefaultPerPage(5);
        $grid->addColumn('login', 'Riešitel');
 ...
 if ($values->created < $values->submitted) {
    ...
    protected function createComponentFilesGrid($name) {
        $grid = new Grid($this, $name);
        $grid->translator->setLang('sk');
        $grid->setModel($this->context->database->table('file')
                ...
                ->where('task.project_id', $this->idproj))
                ->setDefaultPerPage(5);
