...
//$this->task = $this->taskRepository->findAll()->where('task_id', $id)->fetch();
	...
	public function renderDefault()
	{
		...
		$this->template->taskDetail = $this->taskRepository->findAll()->where('id', $this->idtask);
                $this->template->files = $this->context->database->table('file')->select('*')->where('task_id', $this->idtask);
	}
	...
	{$akcia = $this->fileRepository->findBy(array('id' => $fileId))->fetch();
        ...
        }else{$this->flashMessage('Problém pri zmazaní súboru (URL = NULL).', 'error');}
        if (!$this->presenter->isAjax()) {
        ...
        $fileName = $fileDat->filename;
         $httpResponse = $this->context->getService('httpResponse');
           ...
           if( $httpResponse->setHeader('Content-Length', filesize($file)) &&
            ...
            ){$this->flashMessage('Súbor stiahnut7.', 'success');}else{$this->flashMessage('Problém pri sťahovaní súboru.', 'error');}
         if (!$this->presenter->isAjax()) {
...
if($this->context->database->table('file')->select('task_id')->where('task_id',$id )->count() < $this->task->numfiles){
       ...
       /* if ($id > 0) {
           ...
           $inserted = $this->context->database->table('file')->insert(array(
               // 'datumOd' => $form->values->datumOd,
              //  'datumDo' => $form->values->datumDo,
