...
public function inject(Project\UserRepository $userRepository, Project\User_subjRepository $user_subjRepository, Project\SubjectRepository $subjectRepository)
{
...
public function actionView($id) {//$this->project = $this->projectRepository->findAll()->where('project_id', $id)->fetch();
...
protected function createComponentUserViewGrid($name) {
        ...
        $grid->setModel($this->context->database->table('task')
                ...
                ->where('user_subj.user_id', $this->user_id))
                ->setDefaultPerPage(5);
