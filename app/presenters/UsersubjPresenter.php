...
public function inject(Project\User_subjRepository $user_subjRepository,Project\SubjectRepository $subjectRepository)
{
...
protected function createComponentUsersInSubjGrid($name)
{
    ...
    $grid->setModel($this->context->database->table('user_subj')->select('user.login,user.id AS usid, user.name, user.surname, user.email')
