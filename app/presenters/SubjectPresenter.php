...
class SubjectPresenter extends BasePresenter {
    ...
    public function inject(Project\SubjectRepository $subjectRepository, Project\School_yearRepository $school_yearRepository) {
    ...
    protected function createComponentSubjRegForm() {
        ...
        $roky = $this->context->database->table('school_year')->select('id, year')->fetchPairs('id', 'year');
    ...
    protected function createComponentSubjectGrid($name) {
        ...
        $grid->setModel($this->context->database->table('subject')->select('subject.id,subject, school_year.year'))
    ...
    protected function createComponentUsersInSubjGrid($name) {
        $grid = new Grid($this, $name);
        $grid->setModel($this->context->database->table('user_subj')->select('user.login, user.name, user.surname, user.email')
    ...
    protected function createComponentConnectToSubjForm() {
        $users = $this->context->database->table('user')->select('id,login')->fetchPairs('id', 'login');
    ...
    protected function createComponentYearsGrid($name) {
        $grid = new Grid($this, $name);
        $grid->setModel($this->context->database->table('school_year')->select('year, term_start,term_end'))
                ->setDefaultPerPage(10);
